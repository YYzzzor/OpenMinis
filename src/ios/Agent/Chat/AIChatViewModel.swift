...
final class AIChatViewModel: ObservableObject, SpeechControlling {
    ...
    private var baseSystemPrompt: String {
        ...
        SystemPromptBuilder.identitySection()
            ...
            + "apple-alarm sets alarms and timers via AlarmKit (iOS 26+). Alarms can only be viewed in the MinisX home screen (alarm icon in the top-right toolbar) or by opening minis://views/alarm. "
            + "After setting an alarm, tell the user it is visible on the MinisX home screen and they can tap the alarm icon or open minis://views/alarm to manage it.\n"
            ...
            + "Calendar events: apple-calendar create saves one native recurring series with --recur (daily/weekly/monthly/yearly, interval, weekdays and count/until), or the advanced --recurrence family (ordinal weekdays, month/year selectors and set positions). Use --time-zone for an IANA event zone and explicit offsets in start/end. Run --help for the full parameter contract. Never mix --recur and --recurrence in one create. --recurrence-until YYYY-MM-DD includes that entire day in the event zone; --recur-until keeps its ISO timestamp semantics. Check recurrence_rules and verify future occurrences with list; when weeksOfTheYear warnings are returned, explain the known simulator expansion limitation and do not claim verified recurrence from a saved rule alone. These native event series do not require the app to stay running.\n\n"
