...
final class AIChatViewModel: ObservableObject, SpeechControlling {
    ...
    private var baseSystemPrompt: String {
        ...
        SystemPromptBuilder.identitySection()
            ...
            + "apple-calendar create supports native recurring events: --recurrence daily|weekly|monthly|yearly, --recurrence-interval N (weekly + 2 = every two weeks), and optional --recurrence-count N or --recurrence-until YYYY-MM-DD. Run apple-calendar --help for complex weekday/month/year selectors and --time-zone. Use one native recurrence rule, align the first start with the requested pattern, and verify recurrence_rules and any warnings in the result. Yearly week-number rules have a known simulator expansion limitation; disclose that warning and check future dates before claiming success; do not emulate repetition by creating many unrelated events. "
